                    ...
                    <ul class="nav navbar-nav navbar-right">
                        <?php
                             session_start();
								...
								echo $_SESSION['ErabiltzaileMota'];
	...
	if($linki->connect_errno){
		...
		if(!($emaitza)){
			...
			while($row = $emaitza->fetch_assoc()){
				echo ('<tr><td>'.$row['GalderaZenbakia'].'</td><td>'.$row['EgilearenEposta'].'</td><td>'.$row['Gaia'].'</td><td>'.$row['GalderarenTestua'].'</td><td>'.$row['ErantzunTestua'].'</td><td>'.$row['Zailtasuna'].'</td>'  .     '<td><button id="editatuBotoia" name="editatuBotoia" class="btn btn-warning"  data-toggle="modal" onclick="editatu('.$row["GalderaZenbakia"].')"><span class="glyphicon glyphicon-pencil"></span></button></td>' .
				'</td><tr><tr><tr><tr><tr>');
			}
